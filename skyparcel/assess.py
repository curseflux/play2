"""Skyparcel runner. Commands: list, test, replay, stress."""
import argparse
import base64
import copy
...
def load(filename):
    ...
    if not callable(getattr(module, "choose", None)):
        raise ValueError("submission must define choose(view)")
    return module


def run(course, module, record=False):
    world, frames, peak, error = Flight(course), [], 0.0, None
    while not world.done:
        view = copy.deepcopy(world.observe())
        start = time.perf_counter()
        try:
            action = module.choose(view)
...
def main(argv=None):
    ...
    for command in (test, replay, stress):
        command.add_argument("--submission", default="submission.py")
    args = parser.parse_args(argv)
    visible = courses()
    if args.command == "list":
        for c in visible:
            print(f"{c['name']} {c['title']:<12} need {c['required']} deliveries")
        return 0
    ...
    for c in selected:
        ...
        if c["name"].startswith("X"):
            ...
            print(f"{mark} {c['name']} deliveries {r['deliveries']}/{r['required']}  "
